...
namespace BlockYourFriends.Multiplayer.relay
{
    ...
    public class RelayUtpClient : MonoBehaviour, IDisposable // This is a MonoBehaviour merely to have access to Update.
    {
        ...
        protected List<NetworkConnection> connections; // For clients, this has just one member, but for hosts it will have more.
        protected bool IsRelayConnected { get { return localLobby.RelayServer != null; } }

        protected bool hasSentInitialMessage = false;
        private const float HeartbeatPeriod = 15;
        ...
        public virtual void Initialize(NetworkDriver networkDriver, List<NetworkConnection> connections, LobbyUser localUser, LocalLobby localLobby)
        {
            ...
            this.localUser.onChanged += OnLocalChange;
            this.networkDriver = networkDriver;
            this.connections = connections;
            Locator.Get.UpdateSlow.Subscribe(UpdateSlow, HeartbeatPeriod);
        }
        protected virtual void Uninitialize()
        {
            localUser.onChanged -= OnLocalChange;
            Leave();
        ...
        {
            if (connections.Count == 0) // This could be the case for the host alone in the lobby.
                return;
            foreach (NetworkConnection conn in connections)
                DoUserUpdate(networkDriver, conn, this.localUser);
        }
        ...
        {
            if (!IsRelayConnected) // If disconnected from Relay for some reason, we *want* this client to timeout.
                return;
            foreach (NetworkConnection connection in connections)
                WriteByte(networkDriver, connection, "0", MsgType.Ping, 0); // The ID doesn't matter here, so send a minimal number of bytes.
        }

        protected virtual void OnUpdate()
        {
            if (!hasSentInitialMessage)
                ReceiveNetworkEvents(networkDriver); // Just on the first execution, make sure to handle any events that accumulated while completing the connection.
            networkDriver.ScheduleUpdate().Complete(); // This pumps all messages, which pings the Relay allocation and keeps it alive. It should be called no more often than ReceiveNetworkEvents.
            ReceiveNetworkEvents(networkDriver); // This reads the message queue which was just updated.
            if (!hasSentInitialMessage)
                SendInitialMessage(networkDriver, connections[0]); // On a client, the 0th (and only) connection is to the host.
        }
        ...
        private void ProcessNetworkEvent(NetworkConnection conn, DataStreamReader strm, NetworkEvent.Type cmd)
        {
            if (cmd == NetworkEvent.Type.Data)
            {
                ...
                if (msgType == MsgType.PlayerApprovalState)
                {
                    ...
                    if (approval == Approval.OK && !localUser.IsApproved)
                        OnApproved(networkDriver, conn);
        ...
        protected virtual bool CanProcessDataEventFor(NetworkConnection conn, MsgType type, string id)
        {
            ...
            return id != localUser.ID && (localUser.IsApproved && localLobby.LobbyUsers.ContainsKey(id) || type == MsgType.PlayerApprovalState);
        }
        ...
        {
            WriteByte(driver, connection, localUser.ID, MsgType.NewPlayer, 0);
            hasSentInitialMessage = true;
        }
        private void OnApproved(NetworkDriver driver, NetworkConnection connection)
        {
            localUser.IsApproved = true;
            ForceFullUserUpdate(driver, connection, localUser);
        }
        ...
        public virtual void Leave()
        {
            foreach (NetworkConnection connection in connections)
                ...
                WriteByte(networkDriver, connection, localUser.ID, MsgType.PlayerDisconnect, 0);
