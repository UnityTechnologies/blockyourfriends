...
namespace BlockYourFriends.Multiplayer.relay
{
    ...
    public class RelayUtpHost : RelayUtpClient, IReceiveMessages
    {
        public override void Initialize(NetworkDriver networkDriver, List<NetworkConnection> connections, LobbyUser localUser, LocalLobby localLobby)
        {
            ...
            hasSentInitialMessage = true; // The host will be alone in the lobby at first, so they need not send any messages right away.
        ...
        {
            if (!IsRelayConnected) // If Relay was disconnected somehow, stop taking actions that will keep the allocation alive.
                return;
            base.OnUpdate();
            UpdateConnections();
        }
        ...
        {
            WriteByte(networkDriver, conn, localUser.ID, MsgType.PlayerApprovalState, (byte)result);
            if (result == Approval.OK && conn.IsCreated)
            {
                foreach (var user in localLobby.LobbyUsers)
                    ForceFullUserUpdate(networkDriver, conn, user.Value);
                connections.Add(conn);
            }
            else
            {
                conn.Disconnect(networkDriver);
            }
        }
        ...
        protected override bool CanProcessDataEventFor(NetworkConnection conn, MsgType type, string id)
        {
            ...
            return id != localUser.ID && (localLobby.LobbyUsers.ContainsKey(id) && connections.Contains(conn) || type == MsgType.NewPlayer);
        }
        ...
        protected override void ProcessNetworkEventDataAdditional(NetworkConnection conn, MsgType msgType, string id)
        {
            ...
            {
                string name = localLobby.LobbyUsers[id].DisplayName;
                foreach (NetworkConnection otherConn in connections)
                {
                    if (otherConn == conn)
                        continue;
                    WriteString(networkDriver, otherConn, id, msgType, name);
                }
            }
            else if (msgType == MsgType.Emote || msgType == MsgType.ReadyState)
            {
                byte value = msgType == MsgType.Emote ? (byte)localLobby.LobbyUsers[id].Emote : (byte)localLobby.LobbyUsers[id].UserStatus;
                foreach (NetworkConnection otherConn in connections)
                {
                    if (otherConn == conn)
                        continue;
                    WriteByte(networkDriver, otherConn, id, msgType, value);
                }
            }
        ...
        public void OnReceiveMessage(MessageType type, object msg)
        {
            ...
            else if (type == MessageType.EndGame) // This assumes that only the host will have the End Game button available; otherwise, clients need to be able to send this message, too.
            {
                foreach (NetworkConnection connection in connections)
                    WriteByte(networkDriver, connection, localUser.ID, MsgType.EndInGame, 0);
            }
        }
        ...
        private void CheckIfAllUsersReady()
        {
            ...
            if (haveAllReadied && localLobby.State == LobbyState.Lobby) // Need to notify both this client and all others that all players have readied.
            {
                Locator.Get.Messenger.OnReceiveMessage(MessageType.StartCountdown, null);
                foreach (NetworkConnection connection in connections)
                    WriteByte(networkDriver, connection, localUser.ID, MsgType.StartCountdown, 0);
            }
            else if (!haveAllReadied && localLobby.State == LobbyState.CountDown) // Someone cancelled during the countdown, so abort the countdown.
            {
                Locator.Get.Messenger.OnReceiveMessage(MessageType.CancelCountdown, null);
                foreach (NetworkConnection connection in connections)
                    WriteByte(networkDriver, connection, localUser.ID, MsgType.CancelCountdown, 0);
            }
        }
        ...
        public void SendInGameState()
        {
            ...
            foreach (NetworkConnection connection in connections)
                WriteByte(networkDriver, connection, localUser.ID, MsgType.ConfirmInGame, 0);
        }
        ...
        private void UpdateConnections()
        {
            ...
            {
                if (!connections[c].IsCreated)
                    connections.RemoveAt(c);
            }
            while (true)
            {
                var conn = networkDriver.Accept(); // Note that since we pumped the event queue earlier in Update, m_networkDriver has been updated already this frame.
        ...
        {
            foreach (NetworkConnection connection in connections)
                connection.Disconnect(networkDriver); // Note that Lobby won't receive the disconnect immediately, so its auto-disconnect takes 30-40s, if needed.
            connections.Clear();
